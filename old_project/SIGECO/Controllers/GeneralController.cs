...
namespace SIGECO.Controllers
{
    public class GeneralController : Controller
    {
        ...
        [HttpPost]
        public ActionResult ListData(string schema, string table)
        {
            var db = new SIGECO_Entities();
            var consulta = db.spGetXMLRowsByTableName(schema, table)
                .Select(c => c).ToList();
            string xml = consulta.FirstOrDefault().ToString();
